const SET_TOPICS = 'set_topics'
const SET_LAST_EXECUTE = 'set_last_execute'
const LOAD_FAILED = 'load_failed'
const LOAD_EXECUTE_FAILED = 'load_execute_failed'
const SET_FILTER_STRING = 'set_filter_string'
const SET_ORDER = 'set_order_string'
const COMPLETE_LOADING = 'complete_loading'
const SET_TOPIC_PAUSED = 'set_topic_paused'
const SET_TOPIC_STATUS = 'set_topic_status'
const SET_EXECUTING = 'set_executing'
const SET_CURRENT_EXECUTE_LOGS = 'set_current_execute_logs'
const SET_SETTINGS = 'set_settings'
const SET_UPDATE_INTERVAL = 'set_update_interval'
const SET_PROXY_ENABLED = 'set_proxy_enabled'
const SET_PROXY = 'set_proxy'
const SET_NEW_VERSION_CHECKED_ENABLED = 'set_new_version_checked_enabled'
const SET_NEW_VERSION_CHECKED_INCLUDE_PRERELEASE = 'set_new_version_checked_include_prerelease'
const SET_NEW_VERSION_CHECKED_INTERVAL = 'set_new_version_checked_interval'
const SET_TRACKERS = 'set_trackers'
const LOAD_TRACKERS_FAILED = 'load_trackers_failed'
const SET_TRACKER_MODEL = 'set_tracker_model'
const SET_TRACKER_MODEL_SAVING = 'set_tracker_model_saving'
const SET_TRACKER_MODEL_CHECKING = 'set_tracker_model_checking'

export default {
    SET_TOPICS,
    SET_LAST_EXECUTE,
    LOAD_FAILED,
    LOAD_EXECUTE_FAILED,
    SET_FILTER_STRING,
    SET_ORDER,
    COMPLETE_LOADING,
    SET_TOPIC_PAUSED,
    SET_TOPIC_STATUS,
    SET_EXECUTING,
    SET_CURRENT_EXECUTE_LOGS,
    SET_SETTINGS,
    SET_UPDATE_INTERVAL,
    SET_PROXY_ENABLED,
    SET_PROXY,
    SET_NEW_VERSION_CHECKED_ENABLED,
    SET_NEW_VERSION_CHECKED_INCLUDE_PRERELEASE,
    SET_NEW_VERSION_CHECKED_INTERVAL,
    SET_TRACKERS,
    LOAD_TRACKERS_FAILED,
    SET_TRACKER_MODEL,
    SET_TRACKER_MODEL_SAVING,
    SET_TRACKER_MODEL_CHECKING
}
