...
class LostFilmPlugin(WithCredentialsMixin, TrackerPluginBase):
    ...
    _season_title_info = re.compile(u'^(?P<season>\d+)\s+СЃРµР·РѕРЅ')
    ...
    def _parse_season_info(self, info):
        if 'Р”РѕРїРѕР»РЅРёС‚РµР»СЊРЅС‹Рµ РјР°С‚РµСЂРёР°Р»С‹' in info: return SpecialSeasons.Additional
    ...
    def _get_download_info(self, url, scraper, domain, cat, season, episode):
        ...
        try:
            ...
            # РР·РІР»РµРєР°РµРј С‡Р°СЃС‚СЊ URL РёР· РјРµС‚Р°-С‚РµРіР°
            relative_url = meta['content'].split(';')[1].strip()
            if 'url=' in relative_url.lower():
                relative_url = relative_url.split('url=')[1]

            # РЎРѕР·РґР°РµРј Р°Р±СЃРѕР»СЋС‚РЅС‹Р№ URL, РёСЃРїРѕР»СЊР·СѓСЏ Р±Р°Р·РѕРІС‹Р№ URL СЃС‚СЂР°РЅРёС†С‹
    ...
    def prepare_add_topic(self, url):
        with DBSession() as db:
            cred = db.query(self.credentials_class).first()
            # РР·РІР»РµРєР°РµРј quality РґРѕ Р·Р°РєСЂС‹С‚РёСЏ СЃРµСЃСЃРёРё
